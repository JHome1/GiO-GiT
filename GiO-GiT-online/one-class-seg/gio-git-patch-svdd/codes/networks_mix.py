...
class Encoder(nn.Module):
    ...
    def fpath_from_name(name):
        return f'ckpts_mix/{name}/encoder_nohier.pkl'
...
class EncoderDeep(nn.Module):
    ...
    @staticmethod
    def fpath_from_name(name):
        return f'ckpts_mix/{name}/encdeep.pkl'
...
class EncoderHier(nn.Module):
    ...
    @staticmethod
    def fpath_from_name(name):
        return f'ckpts_mix/{name}/enchier.pkl'


class PositionClassifier(nn.Module):
    def __init__(self, K, D, D_emb=128, class_num=8, curvature_s=1.0, curvature_h=1.0):
        super().__init__()
        self.D = D
        ...
        self.fc3_s  = spherenn.AngleLinear(D_emb, class_num, curvature=curvature_s, m=1)

        # define hyperbolic layers
        self.tp         = hypnn.ToPoincare(c=curvature_h, train_x=False, train_c=False, ball_dim=D)
        self.HypLinear1 = hypnn.HypLinear(D, D_emb, c=curvature_h)
        self.HypLinear2 = hypnn.HypLinear(D_emb, D_emb, c=curvature_h)
        self.mlr        = hypnn.HyperbolicMLR(ball_dim=D_emb, n_classes=class_num, c=curvature_h)
    ...
    def fpath_from_name(self, name):
        return f'ckpts_mix/{name}/position_classifier_K{self.K}.pkl'
    ...
    def infer(c, enc, batch):
        ...
        logits, logits_s, logits_h, _, _, _, _, _, _ = c(x1, x2)

        loss1 = xent(logits, ys)
        loss2 = xent(logits_h, ys)
        loss3 = angleloss(logits_s, ys)
    ...
    def forward(self, x1, x2):
        ...
        x_h  = self.HypLinear1(x_h)
        info_h1 = x_h
        x_h  = self.act1(x_h)
        x_h  = self.HypLinear2(x_h)
        info_h2 = x_h
        x_h  = self.act2(x_h)
        x_h  = self.mlr(x_h)
        ...
        return x, x_s, x_h, info_l1, info_s1, info_h1, info_l2, info_s2, info_h2
