...
class Encoder(nn.Module):
    ...
    def fpath_from_name(name):
        return f'ckpts_hyper/{name}/encoder_nohier.pkl'
...
class EncoderDeep(nn.Module):
    ...
    @staticmethod
    def fpath_from_name(name):
        return f'ckpts_hyper/{name}/encdeep.pkl'
...
class EncoderHier(nn.Module):
    ...
    @staticmethod
    def fpath_from_name(name):
        return f'ckpts_hyper/{name}/enchier.pkl'
...
class PositionClassifier(nn.Module):
    def __init__(self, K, D, D_emb=128, class_num=8, curvature=1.0):
        ...
        self.tp         = hypnn.ToPoincare(c=curvature, train_x=False, train_c=False, ball_dim=D)
        self.HypLinear1 = hypnn.HypLinear(D, D_emb, c=curvature)
        self.HypLinear2 = hypnn.HypLinear(D_emb, D_emb, c=curvature)
        self.mlr        = hypnn.HyperbolicMLR(ball_dim=D_emb, n_classes=class_num, c=curvature)
    ...
    def fpath_from_name(self, name):
        return f'ckpts_hyper/{name}/position_classifier_K{self.K}.pkl'
    ...
    def infer(c, enc, batch):
        ...
        logits, logits_h, _, _, _, _ = c(x1, x2)
    ...
    def forward(self, x1, x2):
        ...
        return x, x_h, info_l1, info_h1, info_l2, info_h2
