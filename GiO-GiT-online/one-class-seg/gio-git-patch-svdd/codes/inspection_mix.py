...
def infer(x, enc, cls, K, S, args):
    ...
    d_me = np.empty((dataset.N, dataset.row_num, dataset.col_num), dtype=np.float32)
    ...
    with torch.no_grad():
        for xs, x_jit, ns, iis, js in loader:
            ...
            logit, logit_s, logit_h, _, _, _, _, _, _ = cls(embedding, embedding_jit)
            logit_s = logit_s[0]        

            d_me_tmp = torch.zeros(logit.shape[0])
            for i in range(logit.shape[0]):
                d_se_ele = KL_distance(logit_s[i].unsqueeze(0), logit[i].unsqueeze(0))
                d_he_ele = KL_distance(logit_h[i].unsqueeze(0), logit[i].unsqueeze(0))

                d_me_ele = math.sqrt(d_se_ele**2 + d_he_ele**2)
                d_me_ele = torch.tensor(d_me_ele)
                d_me_ele = torch.tanh(d_me_ele)

                d_me_tmp[i] = 1-d_me_ele # embedding: d_me_ele; logit: 1-d_me_ele
                if not args.use_minus_distance:
                    d_me_tmp[i] = d_me_ele

            d_me_tmp = d_me_tmp.detach().cpu().numpy()

            for d_me_ele, n, i, j in zip(d_me_tmp, ns, iis, js):
                d_me[n, i, j] = d_me_ele
    return d_me
