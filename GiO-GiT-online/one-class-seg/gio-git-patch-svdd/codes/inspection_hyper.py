...
def infer(x, enc, cls, K, S, args):
    ...
    d_he = np.empty((dataset.N, dataset.row_num, dataset.col_num), dtype=np.float32)
    ...
    with torch.no_grad():
        for xs, x_jit, ns, iis, js in loader:
            ...
            logit, logit_h, _, _, _, _ = cls(embedding, embedding_jit)        

            d_he_tmp = torch.zeros(logit.shape[0])
            for i in range(logit.shape[0]):
                d_he_ele = KL_distance(logit_h[i].unsqueeze(0), logit[i].unsqueeze(0))
                d_he_ele = torch.tanh(d_he_ele)
                d_he_tmp[i] = 1-d_he_ele # embedding: d_he_ele; logit: 1-d_he_ele
                if not args.use_minus_distance:
                    d_he_tmp[i] = d_he_ele

            d_he_tmp = d_he_tmp.detach().cpu().numpy()

            for d_he_ele, n, i, j in zip(d_he_tmp, ns, iis, js):
                d_he[n, i, j] = d_he_ele
    return d_he
