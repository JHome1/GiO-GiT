...
import anom_utils
from distance import SoftTarget
...
def evaluate(segmentation_module, loader, cfg, gpu):
    ...
    for batch_data in loader:
        ...
        with torch.no_grad():
            ...
            scores_s = torch.zeros(1, 1, segSize[0], segSize[1])
            scores_s = async_copy_to(scores_s, gpu)

            scores_h = torch.zeros(1, 1, segSize[0], segSize[1])
            scores_h = async_copy_to(scores_h, gpu)
            ...
            for img in img_resized_list:
                ...
                # forward pass
                logits, logits_s, logits_h = segmentation_module(feed_dict, segSize=segSize)

                ######################################################
                scores   = scores + logits / len(cfg.DATASET.imgSizes)
                scores_e = torch.max(scores, dim=1)[0].unsqueeze(0)

                scores_s = scores_s + logits_s / len(cfg.DATASET.imgSizes)
                scores_h = scores_h + logits_h / len(cfg.DATASET.imgSizes)                
                scores_m = scores_e**2 + scores_s**2 + scores_h**2         # for E+S+H
                scores_m = torch.sqrt(scores_m)
                ######################################################
            ...
            #########################################
            if cfg.OOD.use_geo=="use_sphere":
                tmp_scores_s = scores_s
                tmp_geometry_distances = tmp_scores_s
                
            elif cfg.OOD.use_geo=="use_hyper":
                tmp_scores_h = scores_h
                tmp_geometry_distances = tmp_scores_h
  
            elif cfg.OOD.use_geo=="use_mix":
                tmp_scores_m = scores_m
                tmp_geometry_distances = tmp_scores_m
            #########################################
            ...
            if cfg.OOD.ood == "msp":
                ...
                conf = np.max(Q_unary, axis=0).reshape((h,w))
            #######################################################################
            elif cfg.OOD.ood == "curvature-geometry":
                conf = as_numpy(tmp_geometry_distances.squeeze(0).squeeze(0).cpu()) 
            #######################################################################
        ...
        if cfg.VAL.visualize:
            visualize_result((batch_data['img_ori'], seg_label, batch_data['info']),
                             pred,
...
def main(cfg, gpu):
    ...
    # Network Builders
    net_encoder = ModelBuilder.build_encoder(arch=cfg.MODEL.arch_encoder.lower(),
                                             fc_dim=cfg.MODEL.fc_dim,
                                             weights=cfg.MODEL.weights_encoder)
    net_decoder = ModelBuilder.build_decoder(arch=cfg.MODEL.arch_decoder.lower(),
                                             fc_dim=cfg.MODEL.fc_dim,
                                             num_class=cfg.DATASET.num_class,
                                             weights=cfg.MODEL.weights_decoder,
                                             use_softmax=True,
                                             curvature_s=cfg.OOD.curvature_sphere, 
                                             curvature_h=cfg.OOD.curvature_hyper)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--ood",
        help="Choices are [msp, crf-gauss, crf, maxlogit, background, curvature-geometry]",
        default="msp",
    )
    parser.add_argument(
        "--exclude_back",
    ...
    ood = ["OOD.exclude_back", args.exclude_back]     
